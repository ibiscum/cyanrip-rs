...
1. CLI `-B` / `--output-root` value, if provided and non-empty.
...
- `--output-root` overrides `CYANRIP_RS_OUTPUT_ROOT` unconditionally when both are present.
- Legacy compatibility: `--outputroot` remains accepted as an alias.
...
- CLI parse mapping: `src/cli.rs` (`maps_output_root_to_settings`)
...
- Runtime CLI-over-env precedence: `tests/run_workflow_cli.rs` (`run_mode_output_root_cli_overrides_env_output_root`)
