...
pub fn parse_accurip_db_blob(
	...
	while off < data.len() {
		...
		if ntracks != disc_ids.audio_tracks || id1 != disc_ids.id_type_1 || id2 != disc_ids.id_type_2 || cddb != cddb_id {
...
pub fn find_accurip_confidence(status: AccuDbStatus, entries: &[AccuRipDbEntry], checksum: u32, is_450: bool) -> i32 {
...
fn read_u8(data: &[u8], off: &mut usize) -> Result<u8, AccuRipError> {
	if *off >= data.len() {
		return Err(AccuRipError::ParseError("unexpected end of data".to_string()));
	}
	let v = data[*off];
	*off += 1;
	Ok(v)
...
mod tests {
	...
	fn build_valid_blob_for(ids: AccuRipDiscIds, cddb_id: u32) -> Vec<u8> {
		...
		for (c1, ck1, ck1450, c2, ck2, ck2450) in [
			(5u8, 0x11111111u32, 0xAAAAAAAAu32, 3u8, 0x22222222u32, 0xBBBBBBBBu32),
			(9u8, 0x33333333u32, 0xCCCCCCCCu32, 8u8, 0x22222222u32, 0xDDDDDDDDu32),
		] {
	...
	fn find_confidence_matches_c_semantics() {
		...
		assert_eq!(find_accurip_confidence(AccuDbStatus::Found, &entries, 0x1111, false), 7);
		assert_eq!(find_accurip_confidence(AccuDbStatus::Found, &entries, 0xAAAA, true), 2);
		assert_eq!(find_accurip_confidence(AccuDbStatus::Found, &entries, 0x9999, false), -1);
		assert_eq!(find_accurip_confidence(AccuDbStatus::NotFound, &entries, 0x1111, false), 0);
	}
	...
	async fn lookup_maps_404_to_not_found() {
		...
		let out = svc.lookup(&tracks, 0xAABBCCDD).await.expect("lookup should work");
