...
fn map_tracks(medium: &Media, nb_cd_tracks: usize) -> Vec<MusicBrainzTrackMeta> {
    ...
    tracks.iter().take(nb_cd_tracks).map(map_track).collect()
}
...
mod tests {
    ...
    async fn lookup_maps_release_and_tracks_from_wiremock_fixture() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(200).set_body_string(fixture("discid_multi_release.json")),
            )
            .mount(&server)
            .await;
        ...
        assert_eq!(
            out.tracks[1].artist.as_deref(),
            Some("Track Fallback Artist")
        );
    }
    ...
    async fn lookup_handles_invalid_discnumber() {
        ...
        Mock::given(method("GET"))
            ...
            .and(query_param("fmt", "json"))
            .respond_with(
                ResponseTemplate::new(200).set_body_string(fixture("discid_single_release.json")),
            )
            .mount(&server)
            .await;
    ...
    async fn lookup_maps_not_found_from_empty_releases_fixture() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(200).set_body_string(fixture("discid_no_releases.json")),
            )
            .mount(&server)
            .await;
